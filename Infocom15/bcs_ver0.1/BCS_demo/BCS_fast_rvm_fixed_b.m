function [weights,used,sigma2,errbars,basis] = BCS_fast_rvm_fixed_b(PHI,t,B,eta,adaptive,optimal,scale)
%------------------------------------------------------------------
...
alpha = PHI2(index)/(maxr-1/B(index));
% compute initial mu, Sig, S, Q
phi = PHI(:,index);
Hessian = alpha + phi' * diag(B) * phi;
Sig = 1/Hessian;
%mu = Sig*PHIt(index)/sigma2;
mu = Sig * phi' * diag(B) * t;
left = PHI' * phi * B(index);
S = PHI2 * B(index) - Sig * left .^ 2;
Q = PHIt * B(index) - Sig * PHIt(index) * B(index) * left;
%
for count = 1:10000
