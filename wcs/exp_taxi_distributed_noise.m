...
%     M = floor(N * mfrac);
    ...
    run = 0;
    for noisei = 1 : noisen
        noiselevel = noises(noisei);
        fcost = abs(awgn(cost, -noiselevel, 'measured'));
        
        for mfraci = 1 : mfracn
            mfrac = mfracs(mfraci);
            
            run = run + 1;            
            res(run, 1) = mean(abs((fcost-cost)./cost));
            M = floor(N * mfrac);
            ...
            if (sampling == 1)
                if (~exist('alp', 'var') || alp < 0) alp = 1; end
                prob = fcost.^(-alp)*50;
                key = rand(1, N).^(1./prob);
                thr = mfracs(mfraci);
                samplex = find(key>= thr);
                
            elseif (sampling == 2)
                if (kk <= 0) kk = 2; end
                if (~exist('kk', 'var') || kk <= 0) kk = 2; end
                group = round(kk);
                gs = randperm(N);
                gn = min(floor(N/group), M);
                groupwise = zeros(group, gn);
                groupwise(:) = fcost(gs(1:numel(groupwise)));
