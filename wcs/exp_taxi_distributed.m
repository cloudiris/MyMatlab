...
%                 for i = 1 : N
%                     cid = rec(i, 1);
%                     cp = 1 - (1 - prob(i) * fac(cid))^M;
%                     if (binornd(1, cp) == 1)
%                         samplex(i) = 1;
%                         fac(cid) = fac(cid) / (1 - prob(i));
%                     end
%                 end
%                 prob = ones(1, N) - (ones(1, N) - cost.^(-alp) / cest).^M;
%                 prob(find(prob>1)) = 1;
%                 coins = binornd(ones(1, N), prob);
%                 samplex = find(coins==1);
%                 thr = zeros(9000, max(eachM));
%                 minthr = ones(9000, 1);
%                 samplex = zeros(1, N);
%                 weights = cost.^(-alp);
%                 for i = 1 : N
%                     cid = rec(i, 1);
%                     if (eachM(cid) <= 0) continue; end
%                     rn = rand^(-weights(i));
%                     if (rn > thr(cid, minthr(cid)))
%                         samplex(i) = 1;
%                         thr(cid, minthr(cid)) = rn;
%                         [~, minthr(cid)] = min(thr(cid, 1:eachM(cid)));
%                     end
%                 end
