...
% %                 end
            ...
            elseif (sampling == 2.1) %pw-decentralized
                ...
                start = 1;
                for ci = 1 : mm+1
                    if (ci == mm + 1 || rec(in(ci), 6) ~= rec(in(start), 6))
                        ...
                        for cii = 1 : number_in_intv
                            ...
                            % calculate distance
                            lat1 = cand_in_intv(cii, 2) * ones(1, numer_in_intv);
                            lon1 = cand_in_intv(cii, 3) * ones(1, numer_in_intv);
                            lat2 = cand_in_intv(:, 2);
                            lon2 = cand_in_intv(:, 3);
