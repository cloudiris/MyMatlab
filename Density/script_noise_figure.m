...
for d = 1:n_round
    ...
    for i_noise = 1:n_noise
        ...
        [x y_true y sigma mask Num_obs sigma_true] = signal_generator_normal_noise(x_base, A, N, T, K, noise);
        ...
        [y_TBP x_TBP t_TBP] = my_density_csf('TBP', x, y, T, sigma, A, mask, epsilon_TBP(i_noise), thr_TBP(i_noise), Num_obs, Num_tot);
...
%         plot(x_BOUND, 'r');
        ...
        Ey_BP = norm(y_true-y_BP);
        Ey_BCS = norm(y_true-y_BCS);
        Ey_DBP = norm(y_true-y_DBP);      
        Ey_TBP = norm(y_true-y_TBP);
        Ey_BOUND = norm(y_true-y_BOUND);
